...
// POST tambah anggota baru (admin)
...
const updateAnggota = async (req, res) => {
  try {
    ...
    // Update status di tabel anggota
    await db.query(
...
// PUT reset password anggota (admin)
const resetPasswordAnggota = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    res.json({ message: 'Password anggota berhasil direset!' });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Terjadi kesalahan server.' });
  }
...
module.exports = { getAllAnggota, getAnggotaById, createAnggota, updateAnggota, deleteAnggota, resetPasswordAnggota };