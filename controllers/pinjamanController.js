...
const generateCicilan = async (pinjaman_id, jumlah, tenor, bunga) => {
    const cicilanPerBulan = (Number(jumlah) * (Number(bunga) / 100) * Number(tenor) + Number(jumlah)) / Number(tenor);
...
const getAllPinjaman = async (req, res) => {
    try {
        ...
        if (req.user.role === 'admin') {
            [rows] = await db.query(`
        SELECT p.*, a.no_anggota, u.nama, admin.nama as nama_admin
        FROM pinjaman p
...
// PUT setujui/tolak pinjaman (admin)
const prosesPinjaman = async (req, res) => {
    try {
        ...
        const [anggota] = await db.query('SELECT a.user_id FROM anggota a WHERE a.id = ?', [pinjaman[0].anggota_id]);
...
// GET simulasi kalkulator
const simulasiPinjaman = async (req, res) => {
    try {
        const { jumlah, tenor } = req.query;
        const bunga = 1.5;

        if (!jumlah || !tenor) return res.status(400).json({ message: 'Jumlah dan tenor wajib diisi.' });
