...
const bayarCicilan = async (req, res) => {
    ...
    `, [id]);
        ...
        await db.query('UPDATE cicilan SET status = "lunas", tanggal_bayar = NOW() WHERE id = ?', [id]);
...
// PUT update status terlambat
const updateStatusTerlambat = async (req, res) => {
    try {
        const [result] = await db.query(`
      UPDATE cicilan SET status = 'terlambat'
      WHERE status = 'belum_bayar' AND tanggal_jatuh_tempo < CURDATE()
    `);
