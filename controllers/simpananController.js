...
// GET riwayat simpanan (anggota lihat miliknya, admin lihat semua)
...
// GET saldo anggota yang sedang login
...
const setorSimpanan = async (req, res) => {
  try {
    ...
    if (!jumlah || jumlah <= 0) {
      return res.status(400).json({ message: 'Jumlah setor harus lebih dari 0.' });
    }
...
const tarikSimpanan = async (req, res) => {
  try {
    ...
    if (!jumlah || jumlah <= 0) {
      return res.status(400).json({ message: 'Jumlah tarik harus lebih dari 0.' });
    }
